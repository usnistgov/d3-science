  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      if (options.position_cursor) {
        var position_cursor = mainview.append("foreignObject")
    ...
    chart.draw_points = function(data) {
      ...
      series_sel.each((s,i) => {
        ...
        s.forEach((d,i) => {
          let xp = x(d[0]);
          let unplottable_xp = (xp < 0 || xp > width || !isFinite(xp) || d[0] == null || xp == null);
          let yp = y(d[1]);
          let unplottable_yp = (yp < 0 || yp > height || !isFinite(yp) || d[1] == null || yp == null);
    ...
    chart.draw_errorbars = function(data) {
      ...
      let errorbar_width = options.errorbar_width;
      let canvas = this.canvas.node();
      let ctx = canvas.getContext('2d');
      let width = canvas.width;
      let height = canvas.height;
