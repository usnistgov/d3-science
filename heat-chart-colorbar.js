...
function colorbar() {
  ...
  chart.dispatch = dispatch;
  colorbar.redrawScale = function() {
    _redraw_backing = true;
    drawScale(this.mainCanvas);
  };
  
  colorbar.margin = function(_) {
    if (!arguments.length) return margin;
    margin = _;
    return colorbar;
  };
  
  colorbar.width = function(_) {
    if (!arguments.length) return width;
    width = _;
    return colorbar;
  }

  colorbar.ztransform = function(_) {
    if (!arguments.length) return ztransform;
    ztransform = _;
    z = d3.scale[ztransform]();
    _redraw_backing = true;
