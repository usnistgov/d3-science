(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    ...
    global.xyChartCanvas = mod.exports;
  }
})(typeof globalThis !== "undefined" ? globalThis : typeof self !== "undefined" ? self : this, function (_exports, _jqueryExtend, _generateId) {
  "use strict";

  Object.defineProperty(_exports, "__esModule", {
    value: true
  });
  _exports.default = _exports.xyChart = xyChart;
  ...
  function xyChart(options_override) {
    ...
    function do_autoscale() {
      ...
      var set_global = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
      // axis can be 'both', 'x' or 'y'
      var extents;
      var merged_data = d3.merge(source_data);

      if (axis == 'y') {
        var xrange = Array.from(x.domain());
        xrange.sort();
        merged_data = merged_data.filter(function (d) {
          return xrange[0] <= d[0] && xrange[1] >= d[0];
        });
      }

      if (axis == 'x') {
        var yrange = Array.from(y.domain());
        yrange.sort();
        merged_data = merged_data.filter(function (d) {
          return yrange[0] <= d[1] && yrange[1] >= d[1];
        });
      }

      var new_max_x, new_max_y, new_min_x, new_min_y;
    ...
    chart.do_autoscale = do_autoscale; // make it possible to show single data points:
    ...
    function chart(selection) {
      selection.each(function (data) {
        ...
        source_data = data; //chart.update = function() { outercontainer.transition().call(chart); };   
        ...
        yAxis.scale(y).ticks(options.numberOfTicks).tickPadding(10); //zoom.x(x).y(y);

        var canvas_container = outercontainer.append("div").attr("class", "canvas-container").style("position", "relative").style("left", options.margin.left + "px").style("top", options.margin.top + "px"); //************************************************************
        // Generate our SVG object
        //************************************************************

        var svg = outercontainer.append("svg").attr("class", "mainplot"); //.call(zoom) // call this from zoomScroll setter
        ...
        chart.zoom = zoom; //.call(zoom);

        var xaxis = axes.append("g").attr("class", "x axis");
        xaxis.append("rect").attr("height", options.margin.bottom).attr("width", width).style("pointer-events", "all").on("dblclick.zoom", null).on("dblclick.resetzoom", function () {
          chart.resetzoom('x');
        });
        xaxis.append("text").attr("class", "x axis-label").attr("x", width / 2.0).attr("text-anchor", "middle").attr("y", options.margin.bottom - 15);
        var yaxis = axes.append("g").attr("class", "y axis");
        yaxis.append("rect").attr("width", options.margin.left).attr("x", -options.margin.left).attr("height", height).style("pointer-events", "all").on("dblclick.zoom", null).on("dblclick.resetzoom", function () {
          chart.resetzoom('y');
        });
        yaxis.append("text").attr("class", "y axis-label").attr("text-anchor", "middle").attr("transform", "rotate(-90)").attr("y", -options.margin.left + 15).attr("x", -height / 2);
        mainview.append("defs").append("clipPath").attr("id", "d3clip_" + id.toFixed()) // local def
        .append("rect") //.attr("x", 0) // x(min_x)) // options.margin.left)
        //.attr("y", 0)
        .attr("width", width).attr("height", height);
        mainview.attr("clip-path", "url(#d3clip_" + id.toFixed() + ")");
        var canvas = canvas_container.append("canvas").classed("draw-canvas", true).attr("width", width).attr("height", height);
        chart.canvas = canvas;
        axes.append("g").attr("class", "x grid");
        axes.append("g").attr("class", "y grid");
        mainview.append("rect").classed("zoom-box", true).attr("width", width).attr("height", height).style("visibility", "hidden").attr("pointer-events", "all").on("dblclick.zoom", null) //.on("dblclick.resetzoom", null)
        .on("dblclick.resetzoom", function () {
          chart.resetzoom();
        ...
        axes.select(".y.grid").call(yAxisGrid); // remove added attr that blocks styling:
        ...
        if (options.position_cursor) {
          var position_cursor = mainview.append("foreignObject").attr("class", "position-cursor").attr("x", width - 160).attr("y", height - 20).attr("width", 150).attr("height", 20).append("xhtml:div").attr("xmlns", "http://www.w3.org/1999/xhtml").style("text-align", "end");
          ...
          mainview.on("mousemove.position_cursor", follow); //.on("mouseover.position_cursor", follow);
        } //************************************************************
    ...
    chart.draw_legend = function (data) {
      ...
      var el = chart.svg.select("g.legend"); // if there are more options.series defined than datasets, 
      ...
      update_sel.enter().append('g').each(function (d, i) {
        ...
        g.append("rect").attr("x", legend_offset.x).attr("y", i * 25 + 10).attr("width", 14).attr("height", 14).style("cursor", "pointer").on("mouseover", function () {
          ...
          var _options$series$i;

          var hidden = d3.select(this).classed("hidden"); // toggle:

          hidden = !hidden;
          d3.select(this).classed('hidden', hidden);
          var s = options.series[i] = (_options$series$i = options.series[i]) !== null && _options$series$i !== void 0 ? _options$series$i : {};
          ...
          chart.svg.selectAll('g.errorbars')
            ...
            .classed('hidden', hidden);
          */
        }).append("title").text("click to hide/unhide"); //.call(drag_legend);
    ...
    chart.draw_points = function (data) {
      ...
      series_sel.each(function (s, i) {
        ctx.fillStyle = get_series_color(null, i);
        s.forEach(function (d, i) {
          var xp = x(d[0]);
          var unplottable_xp = xp < 0 || xp > width || !isFinite(xp) || d[0] == null || xp == null;
          var yp = y(d[1]);
          var unplottable_yp = yp < 0 || yp > height || !isFinite(yp) || d[1] == null || yp == null;

          if (!unplottable_xp && !unplottable_yp) {
            ctx.beginPath();
            ctx.arc(xp, yp, options.point_size, 0, 2 * Math.PI);
            ctx.closePath();
            ctx.fill();
          }
        });
    ...
    chart.draw_errorbars = function (data) {
      ...
      var height = canvas.height;
      series_sel.each(function (s, i) {
        ctx.strokeStyle = get_series_color(null, i);
        ctx.lineWidth = "1.5";
        ctx.beginPath();
        s.forEach(function (d, i) {
          if (!d[2]) {
          ...
          var px = x(d[0]),
              ...
              puy = y(d[2].yupper),
              ply = y(d[2].ylower);
          var draw_top_bottom = d[2].yupper != d[2].ylower && px >= 0 && px <= width && isFinite(px) && isFinite(ply) && isFinite(puy);
          var draw_left_right = d[2].xupper != d[2].xlower && py >= 0 && py <= height && isFinite(py) && isFinite(plx) && isFinite(pux);

          if (draw_top_bottom) {
            // draw the top bar...
            ctx.moveTo(px - errorbar_width / 2.0, puy);
            ctx.lineTo(px + errorbar_width / 2.0, puy); // draw the vertical...

            ctx.moveTo(px, puy);
            ctx.lineTo(px, ply); // draw the bottom bar

            ctx.moveTo(px - errorbar_width / 2.0, ply);
            ctx.lineTo(px + errorbar_width / 2.0, ply);
          }

          if (draw_left_right) {
            // draw the left bar...
            ctx.moveTo(plx, py - errorbar_width / 2.0);
            ctx.lineTo(plx, py + errorbar_width / 2.0); // draw the horizontal...

            ctx.moveTo(plx, py);
            ctx.lineTo(pux, py); // draw the right bar

            ctx.moveTo(pux, py - errorbar_width / 2.0);
            ctx.lineTo(pux, py + errorbar_width / 2.0);
          }
        });
        ctx.stroke();
    ...
    function update() {
      ...
      svg.select(".y.grid").call(yAxisGrid); // remove added attr that blocks styling:

      svg.selectAll(".grid .tick line").attr("stroke", null);
      svg.selectAll("rect.zoom").remove();
      var canvas = chart.canvas.node();
    ...
    chart.resetzoom = function () {
      ...
      var new_min_x, new_max_x, new_min_y, new_max_y;
      ...
      if (axis == 'both') {
        ...
        orig_x = x.copy();
        orig_y = y.copy();
        chart.g.select("rect.zoom-box").call(zoom.transform, d3.zoomIdentity.translate(xoffset, yoffset).scale(1.0 - 2 * base_zoom_offset));
        is_zoomed = false;
      } else if (axis == 'x') {
        var _do_autoscale = do_autoscale(axis, false),
            _min_x = _do_autoscale.min_x,
            _max_x = _do_autoscale.max_x;

        x.domain([_min_x, _max_x]);
        chart.update();
      } else if (axis == 'y') {
        var _do_autoscale2 = do_autoscale(axis, false),
            _min_y = _do_autoscale2.min_y,
            _max_y = _do_autoscale2.max_y;

        y.domain([_min_y, _max_y]);
        chart.update();
    ...
    chart.zoomScroll = function (_) {
      if (!arguments.length) return zoomScroll;
      zoomScroll = _; //var scrollLayer = chart.svg.select("g.mainview rect");
    ...
    chart.autofit = function () {
      ...
      chart.svg.select("g.mainview").select(".position-cursor").attr("x", width - 160).attr("y", height - 20);
