...
(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    define(["exports", "./jquery-extend.js", "./generate-id.js", "./colormap.js"], factory);
  } else if (typeof exports !== "undefined") {
    factory(exports, require("./jquery-extend.js"), require("./generate-id.js"), require("./colormap.js"));
  } else {
    var mod = {
      exports: {}
    };
    factory(mod.exports, global.jqueryExtend, global.generateId, global.colormap);
    global.heatChartMultiMasked = mod.exports;
  }
})(typeof globalThis !== "undefined" ? globalThis : typeof self !== "undefined" ? self : this, function (_exports, _jqueryExtend, _generateId, lib_colormap) {
  "use strict";

  Object.defineProperty(_exports, "__esModule", {
    value: true
  });
  _exports.default = _exports.heatChartMultiMasked = heatChartMultiMasked;
  lib_colormap = _interopRequireWildcard(lib_colormap);
  ...
  function heatChartMultiMasked(options_override) {
    ...
    var dims = options.dims; // create working copy of zmax and zmin, for zooming colorbar
    ...
    var cb_zoomed = function cb_zoomed() {
      ...
      _redraw_colorbar = true; //chart.redrawImage();
    };
    ...
    function chart(selection) {
      selection.each(function (data) {
        ...
        var limits = fixAspect(width, height); // Update the x-scale.

        x.domain([limits.xmin, limits.xmax]).range([0, width]); // Update the y-scale.

        y.domain([limits.ymin, limits.ymax]).range([height, 0]);
        z.domain([zdims.zmin, zdims.zmax]); // store these for later use.    
        ...
        mainview.append("g").attr("class", "interactor-layer"); // legend on top so it can be moved...

        mainview.append("g").attr("class", "legend").attr("transform", "translate(" + [legend_offset.x, legend_offset.y] + ")");
        mainview.select(".x.axis").call(xAxis);
        mainview.select(".y.axis").call(yAxis);
        mainview.select(".x.grid").call(xAxisGrid);
        mainview.select(".y.grid").call(yAxisGrid); // remove added attr that blocks styling:
    ...
    var drag_legend = d3.drag().container(function () {
      return chart.mainview.node();
    }).on("drag", function (d, i) {
      legend_offset.x += d3.event.dx;
      legend_offset.y += d3.event.dy;
      chart.svg.select("g.legend").attr("transform", "translate(" + [legend_offset.x, legend_offset.y] + ")");
    }).on("start", function () {
    ...
    chart.draw_legend = function (data) {
      ...
      update_sel.enter().append('g').classed('legend-item', true).style("fill", get_series_color).each(function (d, i) {
        var g = d3.select(this);
        g.append("rect").attr("x", 0).attr("y", i * 25).attr("width", 14).attr("height", 14).style("cursor", "pointer").on("mouseover", function () {
        ...
        g.append("text").attr("x", 18).attr("y", i * 25 + 25).attr("height", 30).attr("width", 100).style("text-anchor", "start").style("cursor", "move").on("mouseover", function () {
          chart.svg.selectAll('path.line').classed('highlight', function (d, ii) {
            return ii == i;
          }).classed('unhighlight', function (d, ii) {
            return ii != i;
      ...
      el.selectAll("rect").attr("y", function (d, i) {
        return i * 25 + 12;
      }).style("stroke", get_series_color);
      el.selectAll("text").attr("y", function (d, i) {
        return i * 25 + 25;
      }).each(function (d, i) {
    ...
    chart.colorbar = function (selection) {
      selection.each(function (data) {
        var outercontainer = d3.select(this),
            ...
            height = innerheight - options.cb_margin.top - options.cb_margin.bottom; //colorbar.name = "colorbar";

        chart.colorbar.outercontainer = outercontainer; // update the z axis

        z.range([height, 0]);
        orig_z = z.copy();
        zAxis.scale(z).ticks(options.numberOfTicks).tickPadding(10);
        chart.colorbar.resetzoom = cb_resetzoom;
        chart.colorbar.zoom = cb_zoom; // if inner container doesn't exist, build it.
    ...
    chart.colorbar.update = function () {
      chart.colorbar.colorbarCanvas.call(drawScale);
    ...
    chart.colormap(colormap); //chart.dispatch = dispatch;
    ...
    chart.redrawLoop = function () {
      if (_recalculate_main == true) {
        ...
        make_all_plotdata();
        _redraw_backing = true;
        _redraw_main = true; //drawImage(chart.mainCanvas) //, plotdata);
      }
      ...
      if (_redraw_main == true) {
        ...
        mainview.select(".grid.y").call(yAxisGrid); // remove added attr that blocks styling:
    ...
    chart.aspect_ratio = function (_) {
      ...
      x.domain([limits.xmin, limits.xmax]); // Update the y-scale.
    ...
    chart.position_cursor = function (_) {
      ...
      if (options.position_cursor) {
        ...
        position_cursor.attr("x", parseFloat(mainview.attr("width")) - 10).attr("y", parseFloat(mainview.attr("height")) + options.margin.bottom - 10);

        function get_z(data, dims, x_coord, y_coord) {
          var x_rel = (x_coord - dims.xmin) / (dims.xmax - dims.xmin);
          var y_rel = (y_coord - dims.ymin) / (dims.ymax - dims.ymin);

          if (x_rel >= 1.0 || x_rel < 0 || y_rel >= 1.0 || y_rel < 0) {
            return null;
          } else {
            var x_bin = Math.floor(x_rel * dims.xdim);
            var y_bin = Math.floor(y_rel * dims.ydim); // x_bin and y_bin are int
        ...
        var follow = function follow() {
          ...
          var x_coord = x.invert(mouse[0]),
              y_coord = y.invert(mouse[1]); // start at the top and move down through the datasets:
          ...
          position_cursor.text("x: " + x_coord.toPrecision(5) + ", y: " + y_coord.toPrecision(5) + ", z: " + z_str);
        };
    ...
    function generate_masker() {
      ...
      if (!options.mask.method || options.mask.method == "overlay") {
        var mask_colorname = options.mask.overlay_color || "green";
        var mask_color = lib_colormap.get_html_color(mask_colorname);
        var opacity = options.mask.overlay_opacity || 0.5;
        return function (color, mask_val) {
          var new_color = {};
          var m = mask_val * opacity + 1.0 - opacity;
    ...
    function drawImages(canvas) {
      ...
      if (_redraw_backing) {
        ...
        for (var image_index in source_data) {
          ...
          var maskdata = maskdatas[image_index];
      ...
      source_contexts.forEach(function (ctx, image_index) {
        var _options$series$image;

        if (((_options$series$image = options.series[image_index]) === null || _options$series$image === void 0 ? void 0 : _options$series$image.visible) == false) {
          return;
        }
    ...
    function make_all_plotdata() {
      ...
      var _iterator = _createForOfIteratorHelper(source_data),
          _step;
    ...
    function make_single_maskdata(mask_data, dims) {
      ...
      var height = dims.ydim,
          width = dims.xdim,
          size = height * width; // set the local maskdata:

      var new_maskdata = new Float32Array(size); // source data is an array, but the order can be "F" or "C" (default)
    ...
    function make_single_plotdata(source_data, dims) {
      ...
      var height = dims.ydim,
          width = dims.xdim,
          size = height * width; // set the local plotdata:

      var new_plotdata = new Uint16Array(size); // source data is an array, but the order can be "F" or "C" (default)
      ...
      var image_stride = [1, -width];
      var image_offset = (height - 1) * width;
      var data_stride = f_order ? [1, width] : [height, 1]; //let data_offset = 0;
