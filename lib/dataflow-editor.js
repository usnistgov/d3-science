  ...
  function editor(options) {
    ...
    return function (options) {
      ...
      function reindex_exposed(index_updates) {
        ...
        fields.forEach(reindex); // remove exposed fields when the target module goes away...
      ...
      function update() {
        var id_to_index = {},
            ...
            index_updates = {},
            additions = [],
            removals = []; // need to add module_ids to any modules that are missing them.

        var key_fn = function key_fn(d) {
          if (d.module_id == undefined) {
            var id = (0, _generateId.generateID)();
            Object.defineProperty(d, "module_id", {
              enumerable: false,
              get: function get() {
                return id;
              }
        ...
        module_update.exit()
        /*.each(function(d) {removals.push(d.module_id)})*/
        .remove();
        ...
        exposed_outputs_update.exit().remove(); // remove wires without existing endpoints;
      ...
      Editor.import = function (datum, copyfirst) {
        ...
        Editor.update(); // then import everything else...
      ...
      var wireaction = d3.drag().on("start.wire", wirestart).on("drag.wire", wirepull).on("end.wire", wirestop);
      ...
      function wirestart() {
        ...
        active_wire = true; // add new wire to list of wires in data... but move to "exposed_wires" on stop if needed
      ...
      function module(module_data, index) {
        ...
        if (!('y' in module_data)) module_data.y = 100; // look up terminals from module definition if not in module_data:
        ...
        var drag = d3.drag() //.on("start", function() { orig_x = module_data.x; orig_y = module_data.y })
        .clickDistance(grid_spacing).on("drag", dragmove);
        ...
        var title = group.append("g").classed("title", true); // add title text first so other elements are drawn over it
        ...
        inputs.append("polygon").classed("terminal input state", true).attr("points", "0,0 20," + (height / 2).toFixed() + " 0," + height.toFixed()); // add output elements to group:        
        ...
        outputs.append("polygon").classed("terminal input state", true).attr("points", "0,0 20," + (height / 2).toFixed() + " 0," + height.toFixed()); //add title border last to make sure it's on top

        title.append("rect").classed("title border", true).attr("width", width).attr("height", height).attr("x", 0).attr("y", 0).append("svg:title").text("index: " + index.toFixed());
        group.call(drag);
        return group.node();
      }
      ...
      function exposed_input(input_data, i) {
        var width = 20;
        var height = 20 + options.padding * 2; //var exposed_group = document.createElementNS("http://www.w3.org/2000/svg","g");
      ...
      function resolve_terminal(module_def, side, terminal) {
        ...
        var resolved_terminal = is_target_embedded ? resolve_terminal(target_module_def, side, target_terminal) : target_terminal; // make a copy...
