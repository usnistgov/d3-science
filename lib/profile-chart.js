(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    ...
    factory();
    global.profileChart = mod.exports;
  }
})(this, function () {
  "use strict";
  ...
  var sampleData = [{ "sld_n": 0.000, "sld_m": 0.000, "thickness": 0.0, "theta_m": 270.0 }, { "sld_n": 4.000, "sld_m": 1.000, "thickness": 200.0, "theta_m": 0.0 }, { "sld_n": 2.000, "sld_m": 1.000, "thickness": 200.0, "theta_m": 270.0 }, { "sld_n": 4.000, "sld_m": 0.000, "thickness": 10.0, "theta_m": 270.0 }];
  ...
  function magProfileChart(options_override) {
    ...
    function do_autoscale() {
      ...
      min_y = source_data.reduce(function (a, b) {
        var new_min = Math.min.apply(Math, columns.map(function (sid) {
          return b[sid];
        }));
        return Math.min(a, new_min);
      }, +Infinity);
      max_y = source_data.reduce(function (a, b) {
        var new_max = Math.max.apply(Math, columns.map(function (sid) {
          return b[sid];
        }));
        return Math.max(a, new_max);
      }, -Infinity);
      min_x = 0;
      max_x = source_data.reduce(function (a, b) {
    ...
    function handle_line_dblclick(d, i) {
      ...
      var new_xlist = d.map(function (dd) {
        return dd[0];
      });
      new_xlist.push(xi);
      new_xlist = new_xlist.sort(function (a, b) {
    ...
    function chart(selection) {
      selection.each(function (data) {
        ...
        function apply_constraints(d, i, series, pd) {
          ...
          if (i == pd[0].length - 1 && series == 1) {
            d[1] = 0;
          }
        }

        var point_drag = d3.behavior.drag();
        point_drag.on("dragstart.point", function () {
        ...
        if (options.legend && options.legend.show) {
          ...
          el.selectAll('g').data(data_to_arrays(data)).enter().append('g').each(function (d, i) {
            var g = d3.select(this);
            g.append("rect").attr("x", -options.legend.left).attr("y", i * 25 + 15).attr("width", 10).attr("height", 10).style("fill", colors[i % colors.length]).on("mouseover", function () {
              d3.selectAll('.line')[0][i].classList.add('highlight');
            }).on("mouseout", function () {
              d3.selectAll('.line')[0][i].classList.remove('highlight');
            });

            g.append("text").attr("x", 15 - options.legend.left).attr("y", i * 25 + 25).attr("height", 30).attr("width", 100).style("text-anchor", "start").style("fill", colors[i % colors.length]).on("mouseover", function () {
              d3.selectAll('.line')[0][i].classList.add('highlight');
            }).on("mouseout", function () {
              d3.selectAll('.line')[0][i].classList.remove('highlight');
            });
    ...
    chart.draw_lines = function (data) {
      chart.g.selectAll('.line').data(filterShowOption('show_line', data_to_arrays(data))).enter().append("path").attr("class", "line").attr("series", function (d, i) {
        return i.toFixed();
      }).attr('stroke', function (d, i) {
        return colors[i % colors.length];
      });

      chart.g.selectAll('path.line').attr("d", line).on("dblclick", handle_line_dblclick);
    };
    ...
    chart.draw_points = function (data) {
      chart.g.selectAll("g.series").data(filterShowOption('show_points', data_to_arrays(data))).enter().append("g").attr("class", "series").attr("series", function (d, i) {
        return i.toFixed();
      ...
      update_sel.enter().append("circle").attr("class", "dot").attr("clip-path", "url(#d3clip_" + id.toFixed() + ")").attr("r", options.point_size).call(chart.point_drag);
      update_sel.exit().remove();

      chart.g.selectAll("g.series .dot").each(function (d, i) {
        var xp = x(d[0]),
            unplottable_xp = !isFinite(xp) || d[0] == null || xp == null;
        yp = y(d[1]), unplottable_yp = !isFinite(yp) || d[1] == null || yp == null;
