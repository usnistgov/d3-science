  ...
  function heatChart(options_override) {
    ...
    function chart(selection) {
      selection.each(function (data) {
        ...
        source_data = data; //chart.update = function() { outercontainer.transition().call(chart); chart.colorbar.update(); };   
    ...
    function do_autoscale() {
      ...
      if (!isFinite(new_min_max.min) || !isFinite(new_min_max.max)) {
        new_min_max = {
          min: 1,
          max: 2
        }; // need to put something for invalid input scales.
      }

      zdims.zmin = new_min_max.min;
      zdims.zmax = new_min_max.max;
      z.domain([zdims.zmin, zdims.zmax]); //cb_zoom.y(z);
